...
SELECT Room.BranchID, Room.Floor, Room_Reservation.CheckInDate, Room_Reservation.CheckOutDate, 
CONCAT(Person.FirstName, ' ', Person.LastName) AS CustomerName, Room.RoomNumber
FROM Room
...
SELECT Reservation.ReservationID, Bill.BillingDate, BillDetail.BillDetailName, 
PromotionCode.PromotionCodeName, BillDetail.Charge
FROM Room
INNER JOIN Room_Reservation
ON Room.RoomID = Room_Reservation.RoomID
INNER JOIN Reservation
...
SELECT Customer.CustomerID, Reservation.ReservationID, RoomID, StartDate DATETIME, 
EndDate Datetime, Cancelled
From Customer
INNER JOIN Reservation
...
SELECT r.BranchID, r.RoomNumber, r.StartDate, IFNULL(rt.EndDate, 'Present') AS 'EndDate', 
rt.RoomTypeName
From Room r
